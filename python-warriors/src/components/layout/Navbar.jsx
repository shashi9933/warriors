        ...
        <nav className="fixed top-0 left-0 right-0 z-50 px-4 py-3 bg-deep-space/80 backdrop-blur-md border-b border-glass-border">
            <div className="max-w-7xl mx-auto flex justify-between items-center">
                ...
                <div className="hidden md:flex items-center gap-6">
                    ...
                    {/* Player Rank Badge */}
                    <div className="hidden lg:flex items-center gap-2 px-3 py-1 bg-theme-text/5 border border-theme-text/10 rounded-full">
                        <div className={`w-2 h-2 rounded-full ${getRank(playerData.level).color.replace('text-', 'bg-')}`} />
                        <span className="text-xs font-orbitron text-gray-300">
                            {getRank(playerData.level).title} <span className="opacity-50">Lvl {playerData.level}</span>
                        </span>
                    </div>
                    ...
                    <div className="relative">
                        <button
                            ...
                            className="flex items-center gap-2 px-3 py-2 rounded-lg bg-theme-text/5 border border-theme-text/10 hover:border-neon-cyan/50 transition-colors"
                        >
                            ...
                            <div className="absolute top-full right-0 mt-2 w-40 bg-deep-space border border-glass-border rounded-lg shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
                                {themes.map(t => (
                                    <button
                                        ...
                                        className={`w-full text-left px-4 py-2 text-xs font-orbitron flex items-center gap-2 hover:bg-theme-text/10 ${currentTheme === t.id ? 'text-neon-cyan bg-theme-text/5' : 'text-gray-400'}`}
                                    >
                ...
                <button
                    ...
                    className="md:hidden text-gray-300 hover:theme-text transition-colors"
                >
            ...
            {isOpen && (
                <div className="md:hidden absolute top-full left-0 right-0 bg-deep-space/95 backdrop-blur-xl border-b border-glass-border animate-in slide-in-from-top-2">
                    <div className="flex flex-col p-4 gap-2">
                        ...
                        <div className="border-t border-theme-text/10 my-2 pt-2">
                            ...
                            <div className="grid grid-cols-2 gap-2 px-2">
                                {themes.map(t => (
                                    <button
                                        ...
                                        className={`flex items-center gap-2 px-3 py-2 rounded border text-xs font-orbitron ${currentTheme === t.id ? 'border-neon-cyan text-neon-cyan bg-neon-cyan/10' : 'border-theme-text/10 text-gray-400'}`}
                                    >
                                        <div className={`w-2 h-2 rounded-full ${t.color}`} />
                                        {t.name}
                                    </button>
