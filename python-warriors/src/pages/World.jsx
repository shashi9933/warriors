    ...
    return (
        <PageLayout>
            ...
            <div className="h-[calc(100vh-64px)] md:h-[calc(100vh-140px)] flex flex-col md:flex-row gap-4 md:gap-6 relative">
                ...
                <div className="h-[50vh] md:h-auto md:flex-1 glass-panel p-0 relative overflow-hidden group perspective-[1200px] bg-black/60 shrink-0">
                    ...
                    {NODES.map((node) => (
                        <div
                            ...
                            onClick={() => {
                                setActiveNode(node); // Always select on click
                                playSFX('UI_CLICK');
                                // Only navigate if strictly meant to, but map selection is better first on mobile
                            }}
                        >
                            {/* Floating Node Base */}
                            <motion.div
                                className={`w-16 h-16 md:w-24 md:h-24 rounded-2xl flex items-center justify-center relative transition-all duration-500 
                            ...
                            >
                                ...
                                <node.icon
                                    size={24}
                                    className={`md:w-9 md:h-9 relative z-10 ${node.locked ? 'text-gray-500' : (node.id === activeNode.id ? node.color : 'text-gray-300')} transition-colors duration-300 drop-shadow-[0_5px_5px_rgba(0,0,0,0.5)]`}
                                />
                            ...
                            <div className={`
                                mt-2 md:mt-4 px-2 py-1 md:px-4 md:py-2 bg-black/80 backdrop-blur-sm border rounded-lg text-[10px] md:text-xs font-bold font-orbitron tracking-widest uppercase shadow-xl transition-all duration-300 transform
                ...
                <div className="w-full md:w-96 glass-panel p-4 md:p-6 flex flex-col relative overflow-hidden shrink-0 border-t md:border-t-0 md:border-l border-white/10 flex-1 md:flex-none">
                    <h3 className="font-orbitron theme-text text-sm md:text-lg flex items-center gap-2 mb-4 md:mb-6 border-b border-theme-text/10 pb-4 tracking-wider">
                        <MapPin size={16} className="text-cyan-400 md:w-[18px]" /> CURRICULUM DATA
                    </h3>
                    ...
                    <AnimatePresence mode="wait">
                        ...
                        >
                            ...
                            <div className="flex items-center gap-4 mb-4">
                                <div className={`w-12 h-12 md:w-16 md:h-16 rounded-xl bg-white/5 border border-white/10 flex items-center justify-center ${activeNode.color} shadow-[0_0_20px_rgba(0,0,0,0.3)] shrink-0`}>
                                    <activeNode.icon size={24} className="md:w-8 md:h-8" />
                                </div>
                                <div>
                                    <div className="text-[10px] md:text-xs text-gray-500 font-mono mb-1">MASTERY LEVEL {activeNode.difficulty}</div>
                                    <div className={`text-lg md:text-xl font-black font-orbitron ${activeNode.color} leading-none`}>
                            ...
                            {/* Description */}
                            <div className="text-xs md:text-sm text-gray-300 font-light mb-6 leading-relaxed p-3 md:p-4 bg-black/40 rounded-lg border border-white/5 relative overflow-hidden shrink-0">
                                <div className={`absolute left-0 top-0 bottom-0 w-1 ${activeNode.color.replace('text', 'bg')}`} />
                                {activeNode.desc}
                            </div>

                            {/* Topics List */}
                            <div className="space-y-2 md:space-y-4 mb-4 md:mb-8 shrink-0">
                                <div className="text-[10px] md:text-xs text-gray-500 flex items-center gap-2 font-bold tracking-wider">
                                    <Layers size={12} /> CORE TOPICS
                                </div>
                                <div className="grid grid-cols-2 gap-2">
                                    {activeNode.rewards.map((topic, i) => (
                                        <div key={i} className="flex items-center gap-2 text-[10px] md:text-xs text-gray-400 bg-white/5 px-2 py-2 rounded border border-white/5">
                            ...
                            <div className="mt-auto pt-4">
                                {activeNode.locked ? (
                                    <Button variant="danger" disabled className="w-full h-12 md:h-14 opacity-50 font-bold flex flex-col items-center justify-center gap-1">
                                        <Lock size={14} className="md:w-4" />
                                        <span className="text-xs md:text-sm">MASTERY LOCKED</span>
                                    </Button>
                                ) : (
                                    <Button variant="primary" className="w-full h-12 md:h-14 text-sm md:text-lg font-bold tracking-widest shadow-lg shadow-cyan-500/20 group" onClick={() => navigate(activeNode.path)}>
